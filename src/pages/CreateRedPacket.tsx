...
function CreateRedPacket() {
  ...
  const createRedPacket = async () => {
    ...
    if (parseInt(count, 10) <= 0 || parseInt(count, 10) > 100) {
    ...
    try {
      ...
      const tx = await contract.createRedPacket(parseInt(count, 10), {
        value: amountWei,
      })
      ...
      if (receipt) {
        packetId = receipt.logs[0]?.topics[1]
      }

      alert(
        `红包创建成功! 红包ID: ${packetId ? parseInt(packetId, 16) : '未知'}`,
      )
